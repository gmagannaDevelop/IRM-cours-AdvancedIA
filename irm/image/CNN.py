...
def train_and_test_irm():
  ...
  return l_train1_acc, l_train2_acc, l_test_acc

train1, train2, test = train_and_test_irm()

epoch = np.arange(1,31)
plt.plot(epoch, train1, label="Train 1")
plt.plot(epoch, train2, label="Train 2")
plt.plot(epoch, test, label="Test")
plt.title("Accuracy of the 3 environment with low gradient penalty")
